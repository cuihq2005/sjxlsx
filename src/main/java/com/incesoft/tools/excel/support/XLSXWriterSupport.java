...
public class XLSXWriterSupport extends WriterSupport {
	...
	public void open( String emptyTemplate ) {
		File file = new File( emptyTemplate );
		if( !file.exists( ) ) {
			throw new IllegalStateException( "empty.xlsx not exist in " + emptyTemplate );
		}
		workbook = new SimpleXLSXWorkbook( new File( emptyTemplate ) );
	}
